	...
	for _, name := range stdlibcorpus.Names {
		...
		b.Run(strings.TrimSuffix(name, ".json.zst"), func(b *testing.B) {
			b.Run("valid", func(b *testing.B) {
				...
				b.ReportAllocs()
				for b.Loop() {
					boolSink = vibejson.Valid(src)
				}
			})
			b.Run("dynamic-owned", func(b *testing.B) {
				benchmarkCorpusDynamic(b, src, vibejson.DecoderOptions{})
			})
			b.Run("dynamic-zero-copy", func(b *testing.B) {
				benchmarkCorpusDynamic(b, src, vibejson.DecoderOptions{ZeroCopy: true})
			})
			b.Run("parse-walk", func(b *testing.B) {
				benchmarkCorpusWalk(b, src)
			})
			b.Run("typed-owned", func(b *testing.B) {
				benchmarkCorpusTypedByName(b, name, src, vibejson.DecoderOptions{})
			})
			b.Run("typed-zero-copy", func(b *testing.B) {
				benchmarkCorpusTypedByName(b, name, src, vibejson.DecoderOptions{ZeroCopy: true})
			})
...
func benchmarkCorpusDynamic(b *testing.B, src []byte, opts vibejson.DecoderOptions) {
	decoder, err := vibejson.CompileDecoder[any](opts)
	if err != nil {
		b.Fatal(err)
	}
	var check any
...
func walkValue(v vibejson.Value) int {
	switch v.Kind() {
