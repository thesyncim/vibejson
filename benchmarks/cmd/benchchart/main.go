...
func main() {
	var (
		...
		jsonPath     = flag.String("json", "", "output JSON path")
	...
	if *portablePath == "" || *simdPath == "" || *jsonPath == "" ||
		*timeChart == "" || *bytesChart == "" || *simdChart == "" || *commit == "" ||
	...
	publication, err := buildPublication(all, Metadata{
		Commit:      *commit,
		GoVersion:   *goVersion,
		Machine:     *machine,
		OS:          *goos,
...
func writeJSON(path string, publication Publication) error {
