...
func mustCompile[T any](t *testing.T, opts vibejson.DecoderOptions) vibejson.Decoder[T] {
	t.Helper()
	decoder, err := vibejson.CompileDecoder[T](opts)
	if err != nil {
		t.Fatal(err)
	}
	return decoder
...
func TestCompiledDecoderMatchesStdlib(t *testing.T) {
	...
	decoder := mustCompile[Document](t, vibejson.DecoderOptions{ZeroCopy: true})
...
func TestCompiledDecoderReuseAndOptions(t *testing.T) {
	...
	strict := mustCompile[Document](t, vibejson.DecoderOptions{DisallowUnknownFields: true})
	if err := strict.Decode([]byte(`{"unknown":1}`), &dst); err == nil {
		t.Fatal("strict compiled decoder accepted unknown field")
	}
	caseSensitive := mustCompile[Document](t, vibejson.DecoderOptions{CaseSensitive: true, Replace: true})
...
func TestCompiledDecoderOwnedStringsDoNotAliasInput(t *testing.T) {
	...
	decoder := mustCompile[Document](t, vibejson.DecoderOptions{CaseSensitive: true})
	var dst Document
	if err := decoder.Decode(src, &dst); err != nil {
		t.Fatal(err)
	}
...
func TestCompiledDecoderAllocationContracts(t *testing.T) {
	...
	zeroCopy := mustCompile[Document](t, vibejson.DecoderOptions{ZeroCopy: true, CaseSensitive: true})
	...
	owned := mustCompile[Document](t, vibejson.DecoderOptions{CaseSensitive: true})
	if allocs := testing.AllocsPerRun(1000, func() {
...
func TestCompiledNumericBoundariesMatchStdlib(t *testing.T) {
	...
	var got Numeric
	decoder := mustCompile[Numeric](t, vibejson.DecoderOptions{ZeroCopy: true, CaseSensitive: true})
	if err := decoder.Decode(src, &got); err != nil {
		t.Fatal(err)
	}
	if !reflect.DeepEqual(got, want) {
...
func TestCompiledNumericShortFloatsAndNegativeZero(t *testing.T) {
	...
	decoder := mustCompile[Numeric](t, vibejson.DecoderOptions{CaseSensitive: true})
