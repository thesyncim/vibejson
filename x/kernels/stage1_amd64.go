...
var stage1CursorLoAVX2 = func() (v [32]byte) {
	copy(v[:16], stage1CursorClassLo[:])
	copy(v[16:], stage1CursorClassLo[:])
	return
}()
var stage1CursorHiAVX2 = func() (v [32]byte) {
	copy(v[:16], stage1CursorClassHi[:])
	copy(v[16:], stage1CursorClassHi[:])
	return
}()
...
func stage1BlockAVX2(p *[64]byte, m *Stage1Masks) {
	...
	m.Structural = uint64(c0.BitsToInt8().Greater(ws.BitsToInt8()).ToBits()) | uint64(c1.BitsToInt8().Greater(ws.BitsToInt8()).ToBits())<<32
