...
func checkEncoderParity[T any](t *testing.T, label string, v T) {
	...
	if (gotErr == nil) != (wantErr == nil) {
		t.Errorf("%s: Marshal acceptance differs: vibejson=%v stdlib=%v", label, gotErr, wantErr)
		return
	}
	...
	if (gotErr2 == nil) != (wantErr == nil) {
		t.Errorf("%s: AppendJSON acceptance differs: vibejson=%v stdlib=%v", label, gotErr2, wantErr)
		return
	}
...
func TestNilPointerTextMarshalerMapKey(t *testing.T) {
	...
	if (gotPanic != nil) != (wantPanic != nil) {
		t.Errorf("nil ptr text key: panic differs: vibejson=%v stdlib=%v", gotPanic, wantPanic)
		return
	}
	if (gotErr == nil) != (wantErr == nil) {
		t.Errorf("nil ptr text key: acceptance differs: vibejson=%v stdlib=%v", gotErr, wantErr)
		return
	}
	if gotErr == nil && !bytes.Equal(got, want) {
...
func TestDeepPointerNesting(t *testing.T) {
	for _, depth := range []int{500, 9000, 12000} {
		...
		got, gotErr := Marshal(v)
		if (gotErr == nil) != (wantErr == nil) {
			t.Errorf("depth %d: acceptance differs: vibejson=%v stdlib=%v", depth, gotErr, wantErr)
			continue
		}
		if gotErr == nil && !bytes.Equal(got, want) {
...
func TestMarshalerInvalidUTF8CarveOut(t *testing.T) {
	...
	if gotErr == nil {
		...
		t.Logf("note: vibejson accepted invalid UTF-8 marshaler output (carve-out says reject)")
	}
}
...
func TestPanickingMarshalerPropagates(t *testing.T) {
	...
	if (gotPanic != nil) != (wantPanic != nil) {
		t.Errorf("panic propagation differs: vibejson=%v stdlib=%v", gotPanic, wantPanic)
	}
}
...
func TestControlBytesAllValues(t *testing.T) {
	...
	for c := 0; c < 0x20; c++ {
		...
		if gotErr != nil || !bytes.Equal(got, want) {
			t.Fatalf("control byte %#x: vibejson %q err=%v, stdlib %q", c, got, gotErr, want)
		}
	}
}
...
func TestDepthThresholdAndRoundTrip(t *testing.T) {
	...
	if err := Unmarshal(src, &head); err != nil {
		t.Fatalf("vibejson failed to decode depth-%d doc: %v", depth, err)
	}
	...
	if (gotErr == nil) != (wantErr == nil) {
		t.Errorf("re-encode of own decode at depth %d: vibejson err=%v, stdlib err=%v", depth, gotErr != nil, wantErr)
	}
}
...
func TestDisableHTMLEscapingFieldNames(t *testing.T) {
	...
	if err != nil || !bytes.Equal(got, want) {
		t.Errorf("no-escape field names: vibejson %s err=%v, stdlib %s", got, err, want)
	}
}
