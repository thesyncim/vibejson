...
generated output, tests, and corpora in the current repository. A compact
`Provenance: ID` comment at an implementation site resolves to one row here.
Conceptual similarity is not listed as copied code, and uncertain origins stay
...
Authors' BSD-3-Clause text for identified Go-derived material, and
`LICENSE-SIMDJSON` contains the Apache-2.0 text for identified material derived
from the simdjson project; neither licenses the repository as a whole. A root
`LICENSE` and final `NOTICE` remain release requirements.

## Source and algorithm ledger

| ID | Local material | Authoritative source and license | Local changes and integrity |
| --- | --- | --- | --- |
| `GO-EISEL-001` | `x/floatconv/eisel.go:EiselLemire64`; generated `x/floatconv/eisel_table.go` | Go 1.25.0, commit `6e676ab2b809d46623acb5988248d95d1eb7939c`, `src/strconv/eisel_lemire.go`; Go Authors, BSD-3-Clause, `LICENSE-GO` | Float64-only JSON integration and expanded comments. The `math/big` generator is local; all 696 output rows are mathematically equivalent to Go's table. `go generate` is the integrity check. Upstream records deeper lineage to `fast_double_parser` `644bef4306059d3be01a04e77d3cc84b379c596f`, Wuffs `ba3818cb6b473a2ed0b38ecfc07dbbd3a97e8ae7`, and Nigel Tao's test data `5280dcfccf6d0b02a65ae282dad0b6d9de50e039`. |
...
| `GO-FIELDS-001` | `x/jsonfields/jsonfields.go:{Resolve,validTag}`; `typed_compile.go:compileTypedOmitZero` | Go commit `d468ad3648be469ffc4090e4586c29709182d6b6`, `src/encoding/json/encode.go:{typeFields,isValidTag}`; Go Authors, BSD-3-Clause, `LICENSE-GO` | Compiled-field layout, pointer-hop metadata, packed `omitzero` method dispatch, and opt-in `,inline` support. Struct-field parity tests cover visibility, dominance, and zero omission. |
| `GO-STRING-001` | `encoder_string.go:appendEncodedJSONString`; `encode.go:appendJSONStringBytes`; `marshaler.go:escapeHTMLInPlaceTail` | Conservatively treated as adaptations of Go commit `d468ad3648be469ffc4090e4586c29709182d6b6`, `src/encoding/json/encode.go:appendString`, and pinned Go commit `03845e30f7b73d1703bd8c21017297f6eecb76d6`, `src/encoding/json/indent.go:appendHTMLEscape`; Go Authors, BSD-3-Clause, `LICENSE-GO` | SIMD prefix scanning, fused copying, byte-slice/in-place integration, and Go 1.27 invalid-UTF-8 spelling are local. Encoding parity tests cover the result. |
...
| `GO-FLOATFMT-001` | `simd/float.go`; `x/floatconv/eisel.go:FormatPowerOfTen` | Go commit `03845e30f7b73d1703bd8c21017297f6eecb76d6`, `src/internal/strconv/{uscale.go,ftoa.go,pow10tab.go}`; Go Authors, BSD-3-Clause, `LICENSE-GO` | JSON spelling and caller-owned output specialization. The formatter derives its scale representation exactly from the generated Eisel table, so the module keeps one static source table. It materializes a 696-entry formatter view at startup to retain the direct hot lookup; that retains the former 11 KiB runtime footprint and adds bounded initialization work. Differential formatting and all-entry table-digest tests cover parity. |
...
| `ALGO-DIGITS-001` | `simd/digits.go:parse8DigitsWord`; `number_digits.go:parse8DigitsWord`; `number_array16_arm64.go:parseFixed16Uint64Array` | Exact three-constant reduction: Johnny Lee, [“Fast numeric string to int”](https://johnnylee-sde.github.io/Fast-numeric-string-to-int/) (2016), which credits user `bormand`; also C++ simdjson 4.6.4 commit `1bcf71bd85059ab6574ea1159de9298dcc1212c5`, `include/simdjson/arm64/numberparsing_defs.h`, which preserves Lee's explanation. Related SWAR derivation: Daniel Lemire, “Quickly parsing eight digits” (2018) | Classified as algorithm-derived because the historical local source was not recorded. Exhaustive digit tests and fixed-width array route parity cover the result. |

## Generated material and corpora

| ID | Local material | Source, license, and local treatment |
| --- | --- | --- |
| `GO-CORPUS-001` | `tests/stdlib/testdata`, `tests/stdlib/models.go` | Go commit `03845e30f7b73d1703bd8c21017297f6eecb76d6`; payloads from `src/encoding/json/internal/jsontest/_embed/*.json.zst`, models from `src/encoding/json/internal/jsontest/testdata.go`, and BSD license in `tests/stdlib/testdata/LICENSE`. Payloads are byte-for-byte copies and models are mechanically extracted. `scripts/check-stdlib-corpus.sh` verifies the revision, corpus, and generated models. |
| `JSONTESTSUITE-001` | `testdata/corpora/JSONTestSuite` | `nst/JSONTestSuite` commit `1ef36fa01286573e846ac449e8683f8833c5b26a`, `test_parsing/`; MIT license in the adjacent `LICENSE`; no local corpus changes. `y_` inputs must pass, `n_` inputs must fail, and the strict `i_` policy accepts arbitrary-size number spellings and 500 nested arrays while rejecting BOMs, UTF-16, invalid UTF-8, non-Unicode code points, and invalid surrogate pairs. |
