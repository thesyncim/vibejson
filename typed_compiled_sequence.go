...
// Numeric slices share DecodeArray's concrete loops. The callers check the
// scalar kind, 64-bit width, and eight-byte size before entering these adapters.
// Defined slice and element types have the same pointer-free element layout;
// ordinary Go slice assignment supplies the backing-pointer write barrier.
// Assign even on error to preserve the decoded prefix and failing element.
