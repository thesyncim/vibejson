//go:build simdjson_future_compiler_test && (!go1.27 || go1.28) && goexperiment.simd
...
func TestUnvalidatedCompilerReleaseUsesPortableBackends(t *testing.T) {
	info := Current()
	if info.Enabled || info.StringBackend != "scalar" || info.ParseBackend != "scalar" || info.FormatBackend != "scalar" {
		t.Fatalf("Current() = %+v, want portable backends for an unvalidated compiler release", info)
	}
	if info.StringVectorBytes != 0 || info.ParseVectorBytes != 0 || info.FormatVectorBytes != 0 || info.Features != 0 {
