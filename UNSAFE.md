...
| --- | --- | --- | --- | --- | --- |
| Borrowed and lazy views: `value.go`, `node.go`, `index.go`, `parser.go`, `raw.go`, `stream_reader.go`, `encode.go` | Construct zero-copy strings and slices and navigate compact index entries without reflection or allocation. | Every source range comes from a validated token or checked index entry. `IndexEntry` has compile-time size and offset assertions. Empty inputs never dereference a nil base. | Owning `Value` results retain their source and entries. `Index`, `Node`, `RawValue`, zero-copy results, and stream values borrow documented caller or reader storage. Go pointers remain typed pointers; none are stored as `uintptr`. | `ownership_lifetime_test.go`, `gc_lifetime_test.go`, `lazy_navigation_contract_test.go`, `parser_test.go`, `reader_lifecycle_test.go`, `route_differential_test.go` | `BenchmarkParse`, `BenchmarkBuildIndex`, `BenchmarkGetRaw`, `BenchmarkStreamReadNDJSON`, `BenchmarkStreamDynamicWalk` |
...
- `number_float.go` — `scanTypedFloat64`
- `number_float.go` — `scanTypedFloat64Number`
- `number_float.go` — `scanTypedLeadingZeroFloat64`
- `number_float.go` — `scanTypedSimpleFloat64`
...
- `simd/scan_simd.go` — `hasJSONLineSeparatorFast`
- `simd/scan_simd.go` — `scanEncodedHTMLSpecialSIMD`
- `simd/scan_simd.go` — `scanEncodedHTMLSyntaxSIMD`
- `simd/scan_simd.go` — `scanStringSpecialSIMD`
- `simd/scan_simd.go` — `scanStringSyntaxSIMD`
- `simd/scan_simd.go` — `scanUnicodeEscapeRun`
- `simd/scan_simd.go` — `validUTF8NoLineSeparatorGeneric`
- `simd/scan_simd_amd64.go` — `scanEncodedHTMLSpecialAVX2`
- `simd/scan_simd_amd64.go` — `scanEncodedHTMLSpecialAVX512`
- `simd/scan_simd_amd64.go` — `scanEncodedHTMLSyntaxAVX2`
- `simd/scan_simd_amd64.go` — `scanEncodedHTMLSyntaxAVX512`
- `simd/scan_simd_amd64.go` — `scanStringSpecialAVX2`
...
- `typed.go` — `(Decoder[T]).DecodeArray`
...
- `typed_compiled_record.go` — `(*decoderCursor).decodeCompiledStructStructuralExpected`
- `typed_compiled_record.go` — `(*decoderCursor).decodeCompiledStructStructuralInt64String`
- `typed_compiled_record.go` — `(*decoderCursor).decodeCompiledStructStructuralRecord`
- `typed_compiled_record.go` — `(*decoderCursor).decodeCompiledStructStructuralRecordFloat64x3`
- `typed_compiled_record.go` — `(*decoderCursor).decodeCompiledStructStructuralSliceStruct`
- `typed_compiled_record.go` — `(*decoderCursor).decodeCompiledStructStructuralSlow`
- `typed_compiled_record.go` — `(*decoderCursor).matchObjectFieldExpected`
- `typed_compiled_record.go` — `(*decoderCursor).nextObjectFieldExpectedSlow`
- `typed_compiled_record.go` — `(*decoderCursor).tryCompiledStructStructuralRecordFloat64x3`
- `typed_compiled_record.go` — `resetMissingTypedFields`
...
- `typed_compiled_sequence.go` — `decodeCompiledFloatArray`
...
- `valid_bitmap.go` — `validBitmapPerBlock`
- `valid_bitmap.go` — `validBitmapStreamed`
- `valid_bitmap.go` — `validBitmapWalk`
- `valid_bitmap.go` — `validBitmapWalkTrusted`
- `valid_bitmap_stage2.go` — `validBitmapStreamedAsm`
- `valid_bitmap_stage2.go` — `validBitmapWalkAsm`
- `valid_bitmap_stage2.go` — `validScalarTokenAt`
