...
func TestPointerOnlyMarshalerAddressabilityMatchesStdlib(t *testing.T) {
	...
	for _, value := range values {
		...
		if (gotErr == nil) != (wantErr == nil) {
			t.Fatalf("%T: acceptance differs: vibejson=%v stdlib=%v", value, gotErr, wantErr)
		}
		if !bytes.Equal(got, want) {
			t.Fatalf("%T: vibejson=%s stdlib=%s", value, got, want)
		}
	}
	...
	if (gotErr == nil) != (wantErr == nil) || !bytes.Equal(got, want) {
		t.Fatalf("addressable root: vibejson=%s/%v stdlib=%s/%v", got, gotErr, want, wantErr)
	}
	...
	if (gotErr == nil) != (wantErr == nil) || !bytes.Equal(got, want) {
		t.Fatalf("addressable slice element: vibejson=%s/%v stdlib=%s/%v", got, gotErr, want, wantErr)
	}
	...
	if (gotErr == nil) != (wantErr == nil) || !bytes.Equal(got, want) {
		t.Fatalf("mixed addressable methods: vibejson=%s/%v stdlib=%s/%v", got, gotErr, want, wantErr)
	}
	mixedMap := map[string]mixedAddressMarshaler{"k": {}}
	want, wantErr = json.Marshal(mixedMap)
	got, gotErr = Marshal(&mixedMap)
	if (gotErr == nil) != (wantErr == nil) || !bytes.Equal(got, want) {
		t.Fatalf("mixed non-addressable methods: vibejson=%s/%v stdlib=%s/%v", got, gotErr, want, wantErr)
	}
}
