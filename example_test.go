...
func ExampleUnmarshal() {
	...
	if err := vibejson.Unmarshal([]byte(`{"id":7,"name":"launch","enabled":true}`), &event); err != nil {
		panic(err)
	}
...
func ExampleDecodeError() {
	...
	var dst batch
	err := vibejson.Unmarshal([]byte(`{"events":[{"id":1},{"id":"two"}]}`), &dst)
	...
	// Output:
	// events[1].id
	// vibejson: cannot decode JSON at byte 26 into int at events[1].id: expected number
}

func ExampleCompileDecoder() {
	decoder, err := vibejson.CompileDecoder[exampleEvent](vibejson.DecoderOptions{
		ZeroCopy:      true,
		CaseSensitive: true,
	})
	if err != nil {
...
func ExampleGetRaw() {
	...
	tag, ok, err := vibejson.GetRaw(src, "/user/tags/1")
	if err != nil {
		panic(err)
	}
	if !ok {
...
func ExampleUnmarshal_dynamic() {
	var value any
	if err := vibejson.Unmarshal([]byte(`{"name":"ada","scores":[1,2.5]}`), &value); err != nil {
		panic(err)
	}
...
func ExampleValid() {
	fmt.Println(vibejson.Valid([]byte(`{"strict":true}`)))
	fmt.Println(vibejson.Valid([]byte(`{"trailing":1,}`)))
	// Output:
	// true
	// false
}
...
func ExampleAppendCompact() {
	compact, err := vibejson.AppendCompact(nil, []byte("{\n  \"a\": [1, 2]\n}"))
	if err != nil {
		panic(err)
	}
...
func ExampleBuildIndex() {
	src := []byte(`{"items":[{"id":7}]}`)
	var storage [8]vibejson.IndexEntry

	index, err := vibejson.BuildIndex(src, storage[:])
	if err != nil {
		panic(err)
	}
	id, ok, err := index.PointerCompiled(vibejson.MustCompilePointer("/items/0/id"))
	if err != nil {
		panic(err)
	}
	if !ok {
...
func ExampleParse() {
	root, err := vibejson.Parse([]byte(`{"user":{"name":"ada"},"active":true}`))
	if err != nil {
		panic(err)
	}
...
func ExampleReader_Cursor() {
	r := vibejson.NewReader(bytes.NewBufferString("{\"id\":1}\n{\"id\":2}\n"))
	for r.Next() {
		cursor := r.Cursor()
...
func ExampleDecoderOptions() {
	decoder, err := vibejson.CompileDecoder[exampleEvent](vibejson.DecoderOptions{Replace: true})
	if err != nil {
		panic(err)
	}
...
func ExampleCompileEncoder() {
	...
	encoder, err := vibejson.CompileEncoder[stamped](vibejson.EncoderOptions{})
	if err != nil {
		panic(err)
	}
...
func ExampleDecodeNext() {
	encoder, err := vibejson.CompileEncoder[exampleEvent](vibejson.EncoderOptions{})
	if err != nil {
		panic(err)
	}
	decoder, err := vibejson.CompileDecoder[exampleEvent](vibejson.DecoderOptions{})
	...
	w := vibejson.NewWriter(&stream)
	for _, event := range []exampleEvent{{ID: 1, Name: "boot"}, {ID: 2, Name: "run"}} {
		if err := vibejson.EncodeTo(w, encoder, &event); err != nil {
			panic(err)
		}
		w.Newline()
	}
	...
	r := vibejson.NewReader(&stream)
	var event exampleEvent
	for vibejson.DecodeNext(r, decoder, &event) {
...
func ExampleUnmarshalerSimd() {
	...
	if err := vibejson.Unmarshal([]byte(`{"x":3,"note":"ignored","y":4}`), &point); err != nil {
		panic(err)
	}
